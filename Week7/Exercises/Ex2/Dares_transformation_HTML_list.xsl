...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            <xd:p><xd:b>Created on:</xd:b> 15 July 15 2019.</xd:p>
            <xd:p><xd:b>Author:</xd:b> Katarzyna Anna Kapitan</xd:p>
            <xd:p>Stylesheet for the ESU DH course Manuscripts in the Digital Age. It converts TEI-XML encoded
                file with tagged people, places, and numbers into a list of stats in HTML.</xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:template match="//TEI">
        <html>
            ...
            <body>
                ...
                <div>We can also generate data input for mapping software, such as Paladio. 
                    <ul>
                        ...
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <li>
                                    ...
                                    <xsl:variable name="geoRef" select="//place[@xml:id = $REF]//geo"/>
                                    <xsl:variable name="geoRefFormat" select="translate($geoRef, ' ', ',')"/>
                                    <xsl:value-of select="$geoRefFormat"/>
                                    <xsl:text>",</xsl:text>
                                    <xsl:value-of select="following-sibling::num[1]/@value"/>
                                </li></xsl:otherwise>
                        </xsl:choose>
